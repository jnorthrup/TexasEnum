...
//
        ...
        dealer.deck.setCards(
                new int[]{
                        ...
                        card(FIVE, SPADES), card(JACK, DIAMONDS), card(JACK, HEARTS)});


        GameState.start(dealer);

        final int[] deck = dealer.deck.getCards();
        final int[] dst = new int[deck.length];
        System.arraycopy(deck, 0, dst, 0, deck.length);
        final int[] ints = new int[deck.length];
        Integer b = 0;
